...
Check <- function(Data=NULL, silent=FALSE) {
  ...
  if (any(!is.finite(Data$CAW_ESS)))
    stop('Some missing values in the weight composition data')

  time_df <- data.frame(Year=Data$Year, Month=Data$Month)
  # checks for length of time-series data
