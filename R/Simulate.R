...
#' Write Simulated Data to an Excel File
#'
#' @param Simulation An object of class `Simulation`
#' @param xlfile The full path (including extension) of Excel file to write
#' @param n_years The number of years of data to use. Not used if `Sampling` is provided
#' @param sim Simulation number
#' @param Sampling An optional object of class `Simulation`. Defaults to `SLAM::Simulation`
#' @param binned Logical. Should the CAW data be binned?
#' @param dev Logical. Development?
#'
#' @return `xlfile` is written to disk
#' @export
Write_Data2XL <- function(Simulation, xlfile, n_years=3, sim=1, Sampling=NULL, binned=FALSE, dev=F) {
  if (dev) {
    dir <- 'inst'
  } else {
  ...
  ind <- match('Ages', Vars)
  nCol <- length(Simulation$LifeHistory$Weight_Age_Mean)+1
  df_out[ind,2:nCol] <- seq(0, by=1, length.out=length(Simulation$LifeHistory$Weight_Age_Mean))
  Vars2 <- Vars[!Vars=='Ages']
  Vars2 <- Vars[!Vars=='steepness']
  for (v in Vars2) {
    df_out[match(v, Vars),2:nCol] <- Simulation$LifeHistory[[v]]
  }
  ...
  maxW <- exp(log(max(Simulation$LifeHistory$Weight_Age_Mean)) + 1.96*Simulation$LifeHistory$Weight_Age_SD[n_age])
  Weight_Bins <- seq(0, ceiling(maxW), by=0.01)
  nBins <- length(Weight_Bins)-1
  Weight_Mids <- seq(0.5*0.01, by=0.01, length.out=nBins)

  SimDF <- Simulation$At_Age_Time_Series %>% filter(Sim==sim)
  Years <- SimDF$Year %>% unique()
  nyears <- length(Years)
  n_years <- min(n_years, nyears)
  samp_years <- tail(Years, n_years)

  if (!binned) {
    # Calculate Age-Weight Key (assume log-normal variability in weight-at-age)

    AWK <- matrix(0, nrow=n_age, ncol=nBins)
    mu <- log(Simulation$LifeHistory$Weight_Age_Mean) -0.5*Simulation$LifeHistory$Weight_Age_SD^2
    AWK[,1] <- plnorm(Weight_Bins[2], mu, Simulation$LifeHistory$Weight_Age_SD) # probability of weight-at-age
    for (i in 2:(nBins-1)) {
      AWK[,i] <- plnorm(Weight_Bins[i+1], mu, Simulation$LifeHistory$Weight_Age_SD) -
        plnorm(Weight_Bins[i], mu, Simulation$LifeHistory$Weight_Age_SD)
    }
    AWK[,nBins] <- 1 - plnorm(Weight_Bins[nBins], mu, Simulation$LifeHistory$Weight_Age_SD)


    SampDF <- SimDF %>% filter(Year %in% samp_years) %>%
      select(Year, Month, Age, Catch_n)

    Sample_CAW <- function(year, month, SampDF, n_month=150) {
      df <- SampDF %>% filter(Year==year, Month==month)
      prob <- apply(df$Catch_n * AWK, 2, sum)
      prob <- prob/sum(prob)
      sample(Weight_Mids, n_month, prob=prob, replace=T)
    }
    ...
    for (year_i in seq_along(samp_years)) {
      ...
      for (month_i in seq_along(months)) {
        tList[[month_i]] <- data.frame(Year=samp_years[year_i],
                                       ...
                                       Weight=Sample_CAW(samp_years[year_i], months[month_i], SampDF))
      }
      outList[[year_i]] <- do.call('rbind', tList)
    }
    df_out <- do.call('rbind', outList)

    xlsx::write.xlsx(df_out, xlfile, sheetName="CAW-Data", row.names = FALSE, append=TRUE)
  } else {
    Sampled_Data <- Import_Data(Data)
  ...
  # Effort-Data & Index-Data
  df <- Data$Data$TS %>% filter(Sim==sim) %>% select(Year, Month,
                                                     Index_Mean=CPUE,
                                                     Effort_Mean=Effort)
  df$Index_SD <- Data$Sampling$CPUE_CV
  df$Effort_SD <- Data$Sampling$Effort_CV
  df <- df %>% select(Year, Month, Index_Mean, Index_SD, Effort_Mean, Effort_SD)

  xlsx::write.xlsx(df, xlfile, sheetName="Time-Series-Data", row.names = FALSE, append=TRUE)
